...
async function startBot() {
    console.log("🚀 Starting WhatsApp Anti-Link Bot v2.2 - Fresh Start")
    ...
    sock.ev.on("connection.update", async (update) => {
        ...
        if (!state.creds.registered) {
            ...
            if (!cleanNumber) {
                console.log("❌ No phone number found. Set PHONE_NUMBER in Environment Variables")
                process.exit(1)
            }
    ...
    sock.ev.on("messages.upsert", async (m) => {
        try {
            ...
            if (isAdmin && text.startsWith('!')) {
                ...
                if (cmd === 'listwords') {
                    return await sock.sendMessage(from, { text: `*Banned Words:*\n${settings.badwords.join('\n')}` })
                }
                if (cmd === 'warnings') {
                    const mentioned = msg.message.extendedTextMessage?.contextInfo?.mentionedJid?.[0]
                    if (!mentioned) return await sock.sendMessage(from, { text: "❌ Tag a user:!warnings @user" })
                    const key = `${from}@${mentioned}`
                    const count = warnings[key]?.count || 0
                    return await sock.sendMessage(from, { text: `⚠️ @${mentioned.split('@')[0]} has ${count}/${settings.warnKick} warnings`, mentions: [mentioned] })
                }
                if (cmd === 'delwarn') {
                    const mentioned = msg.message.extendedTextMessage?.contextInfo?.mentionedJid?.[0]
                    if (!mentioned) return await sock.sendMessage(from, { text: "❌ Tag a user:!delwarn @user" })
                    delete warnings[`${from}@${mentioned}`]
                    saveWarnings()
                    return await sock.sendMessage(from, { text: `✅ Warnings reset for @${mentioned.split('@')[0]}`, mentions: [mentioned] })
                }
                if (cmd === 'help') {
